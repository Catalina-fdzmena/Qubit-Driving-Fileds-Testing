...
Modified on: 12/11/2023 by: Andrea Catalina Fernandez Mena & Felix Abdiel Rodriguez Rios.
...
# Re[[Ln R_0]']
x = float(input("Please enter an integer for the real part of the logarithmic derivative of R_0:"))
...
def R(t_values):
    eta_t = eta(t_values)
    return (-1j * g1 * np.exp((x + 1j * delta1) * eta_t)) / ((np.cos(Omega1_1 * t_values) ** 2) + kappa1 ** 2 * np.sin(Omega1_1 * t_values) ** 2)

# R(t) function for b)
# def R(t_values):
#     eta_t = eta(t_values)
#     return (-1j * g1 * np.exp((x + 1j * delta2) * eta_t)) / ((np.cos(Omega1_2 * t_values) ** 2) + kappa2 ** 2 * np.sin(Omega1_2 * t_values) ** 2)

# R(t) function for c)
# def R(t_values):
#     eta_t = eta(t_values)
#     return (-1j * g2 * np.exp((x + 1j * delta3) * eta_t)) / ((np.cos(Omega1_3 * t_values) ** 2) + kappa3 ** 2 * np.sin(Omega1_3 * t_values) ** 2)

# R(t) function for d)
# def R(t_values):
#     eta_t = eta(t_values)
#     return (-1j * g2 * np.exp((x + 1j * delta4) * eta_t)) / ((np.cos(Omega1_4 * t_values) ** 2) + kappa4 ** 2 * np.sin(Omega1_4 * t_values) ** 2)

# R(t) function for a) (Periodicity condition not fulfilled)
# def R(t_values):
#     eta_t = eta(t_values)
#     return (-1j * g3 * np.exp((x + 1j * delta5) * eta_t)) / ((np.cos(Omega1_5 * t_values) ** 2) + kappa5 ** 2 * np.sin(Omega1_5 * t_values) ** 2)

# R(t) function for b) (Periodicity condition not fulfilled)
# def R(t_values):
#     eta_t = eta(t_values)
#     return (-1j * g3 * np.exp((x + 1j * delta6) * eta_t)) / ((np.cos(Omega1_6 * t_values) ** 2) + kappa6 ** 2 * np.sin(Omega1_6 * t_values) ** 2)
...
# Time values
t_values = np.linspace(0, Delta1, 1000)
...
x = radius * np.cos(theta)
y = radius * np.sin(theta)
plt.plot(y, x, color='gray', alpha=0.5)

# Population graph
plt.subplot(122)
plt.plot(time, population_values, color='green')
