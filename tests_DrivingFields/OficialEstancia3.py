...
The Case studied in this algorithm is Ω(t)= iΩ_1
...
def eta(t_values):
    ...
    for i, t in enumerate(t_values):
        ...
        integrand = 1 / ((np.cosh(Omega1_1 * s_values)) ** 2 + (kappa1 ** 2) * (np.sinh(Omega1_1 * s_values))** 2)
...
def R(t_values):
    eta_t = eta(t_values)
    return (-1j * g1 * np.exp((x1 + 1j * delta1) * eta_t)) / ((np.cosh(Omega1_1 * t_values) ** 2) + kappa1 ** 2 * np.sinh(Omega1_1 * t_values) ** 2)
...
def P(time):
    return ((4*(Omega1_1**2)*(np.cosh(Omega1_1*time)**2)+((-(x1**2+ 2*1j*x1*delta1 - delta1**2)-(4*(g1**2)))*(np.sinh(Omega1_1*time)**2)))/(((4*(Omega1_1**2))*((np.cosh(Omega1_1*time)**2)+(kappa1**2)*(np.sinh(Omega1_1*time)**2)))))
            
# Time values
t_values = np.linspace(0, 10, 1000)
time = np.linspace(0,10*np.pi/Omega1_1, 1000)
...
plt.xlabel('Time')
plt.ylabel('P(t)')
plt.xticks([0, math.pi/Omega1_1, 5 * math.pi/Omega1_1, 10 * math.pi/Omega1_1],
           ['0', 'π/Ω', '5π/Ω', '10π/Ω'])
